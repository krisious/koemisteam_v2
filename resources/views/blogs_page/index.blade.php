...
@section('content')
    <container>
        <container class="flex p-15 items-center bg-cover bg-center bg-no-repeat h-[40rem]" style="background-image: url('{{ asset('/bg-blog.png') }}')">
            <div class="flex-col pl-30 max-w-3xl">
                ...
                <div class="flex flex-col w-max mb-8 relative z-50">
                    ...
                    <!-- Form Search -->
                    <form method="GET" action="{{ route('blog.index') }}" class="flex items-center space-x-0">
                        <input
                            type="text"
                            name="search"
                            placeholder="Search..."
                    ...
                    @if ($search)
                        <p class="mt-2 text-sm text-gray-600">
                            ...
                            <a href="{{ route('blog.index') }}" class="ml-4 text-blue-600 hover:underline">Reset</a>
                        </p>
                    @endif
                    ...
                    <div class="flex space-x-4 mt-6 text-[#FAFAF6]">
                        ...
                        <div class="relative">
                            ...
                            >
                                <a href="{{ route('blog.index', array_merge(request()->except('filter_modified'), ['filter_modified' => 'latest'])) }}"
                                    class="block px-4 py-2 hover:bg-gray-100 {{ $filterModified == 'latest' ? 'font-bold' : '' }}">
                                    Latest
                                </a>
                                <a href="{{ route('blog.index', array_merge(request()->except('filter_modified'), ['filter_modified' => 'oldest'])) }}"
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                @endphp
                                <a href="{{ route('blog.index', array_merge(request()->except('filter_category'), ['filter_category' => ''])) }}"
                                    class="block px-4 py-2 hover:bg-gray-100 {{ $filterCategory == '' ? 'font-bold' : '' }}">
                                    All
                                </a>
                                @foreach ($categories as $cat)
                                    <a href="{{ route('blog.index', array_merge(request()->except('filter_category'), ['filter_category' => $cat])) }}"
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                @endphp
                                <a href="{{ route('blog.index', array_merge(request()->except('filter_tag'), ['filter_tag' => ''])) }}"
                                    class="block px-4 py-2 hover:bg-gray-100 {{ $filterTag == '' ? 'font-bold' : '' }}">
                                    All
                                </a>
                                @foreach ($tags as $tag)
                                    <a href="{{ route('blog.index', array_merge(request()->except('filter_tag'), ['filter_tag' => $tag])) }}"
